...
public class TripServiceImpl implements TripService {
    ...
    @Override
    @Transactional(readOnly = true)
    public TripDTOs.TripResponse getTripByIdAndFilters(Long id, String routeId, String status) {
        var trip = repo.findByIdAndFilters(id, routeId, status)
                .orElseThrow(() -> new NotFoundException("Trip %d with specified filters not found".formatted(id)));
    ...
    public List<TicketDTOs.TicketResponse> getTicketsByTripIdAndStatus(Long id, TicketStatus status){
        var trip = repo.findById(id)
                .orElseThrow(() -> new NotFoundException("Trip %d not found".formatted(id)));
        return ticketRepo.findByTripIdAndStatus(trip.getId(), status).stream()
   ...
   @Transactional(readOnly = true)
    public List<SeatDTOs.SeatResponse> getSeatsByTripId(Long id, String status) {
         var trip = repo.findById(id)
                .orElseThrow(() -> new NotFoundException("Trip %d not found".formatted(id)));
         return seatRepo.findByTripIdAndStatus(trip.getId(), status).stream()
    ...
    public List<AssignmentDTOs.AssignmentResponse> getAssignmentsByTripId(Long id) {
        var trip = repo.findById(id)
                .orElseThrow(() -> new NotFoundException("Trip %d not found".formatted(id)));
        return assignmentRepo.findByTripId(trip.getId()).stream()
